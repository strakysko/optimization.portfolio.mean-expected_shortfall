...
class QUBOMeanESPortfolio:
    ...
    def __init__(self, returns_data: np.ndarray, prices: np.ndarray,
                 ...
                 alpha: float = 0.95, lambda_risk: float = 1.0):
        """
        ...
        self.J = 8  # bits for ξ₀
        self.K = 8  # bits for τ
        self.L = 8  # bits for y_t

        # self.J = 1  # bits for ξ₀
        # self.K = 1  # bits for τ
        # self.L = 1  # bits for y_t
    ...
    def _add_unbalanced_penalties(self, bqm: dimod.BinaryQuadraticModel, variables: Dict,
        ...
        for t in range(self.T):
            # Get returns for time t (including risk-free rate for ξ₀)
            r_t = np.concatenate([[self.r], self.returns_data[t, :]])
            ...
            for k in range(self.K):
                coeff = -lambda1 * (self.C + scale_tau * (2 ** k))
            ...
            for l in range(self.L):
                for k in range(self.K):
                    coeff = 2 * lambda2 * scale_y * (2 ** l) * (self.C + scale_tau * (2 ** k))
                    if coeff != 0:
            ...
            for j in range(self.J):
                for k in range(self.K):
                    coeff = 2 * lambda2 * scale_xi0 * (2 ** j) * self.r * (self.C + scale_tau * (2 ** k))
                    bqm.add_interaction(variables['a'][j], variables['b'][k], coeff)

            # ξᵢ × τ terms
            for i in range(self.d):
                for k in range(self.K):
                    coeff = 2 * lambda2 * self.returns_data[t, i] * (self.C + scale_tau * (2 ** k))
                    bqm.add_interaction(variables['xi'][i], variables['b'][k], coeff)

            # τ × τ terms
            for k1 in range(self.K):
                for k2 in range(self.K):
                    coeff = lambda2 * (self.C + scale_tau * (2 ** k1)) * (self.C + scale_tau * (2 ** k2))
                    if k1 == k2:
